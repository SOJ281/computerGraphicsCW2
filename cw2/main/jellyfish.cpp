...
JellyModel make_jellyfish(std::size_t legs, Mat44f aPreTransform ) {
    ...
    for (int i = 0; i < legs; i++) {
        ...
        leg = make_change(leg, make_scaling( .05f, 10.f, .05f ));
        leg = make_change(leg, make_translation( {x, -11.f, z }));
...
SimpleMeshData createLeg(Mat44f aPreTransform) {
    ...
    for (float i = 6; i < 0 ; i--) {
        ...
        segment = make_change(segment, make_translation( {0.f, -1.f-i, 0.f }));
        concatenate( leg, segment );
    }
    return leg;
